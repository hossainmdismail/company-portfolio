        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('testimonial.update', $testimonial->id) }}" method="POST">
                    ...
                    <div class="my-3">
                        <select name="user_id" id="" class="form-control">
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" {{ $testimonial->user_id == $user->id?'selected':'' }}>{{ $user->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="custom-file">
                        <input type="file" class="custom-file-input" name="profile" placeholder="Profile" value="{{ $testimonial->profile }}">
                    ...
                    <div class="my-3">
                        <input type="text" class="form-control" name="field" placeholder="Field" value="{{ $testimonial->field }}">
                    </div>
                    <div class="my-3">
