                ...
                <div class="card-body">
                    <table class="table table-striped">
                        ...
                        @foreach ($serviceItems as $service)
                            <tr>
                                <td class="font-weight-bold">{{ $service->service->title }}</td>
                                <td>{{ $service->title }}</td>
                                <td>
                                    <img width="70px" src="{{ asset('uploads/service') }}/{{ $service->image }}"
                                        alt="">
                                </td>
                                <td>{{ $service->description }}</td>
                                <td class="d-flex align-items-center">
                                    <span
                                        class="badge badge-xs light badge-{{ $service->status != 2 ? 'success' : 'warning' }}">{{ $service->status != 2 ? 'Active' : 'Deactive' }}</span>
                                </td>
                                <td>
                                    <div class="d-flex">
                                        <a href="{{ route('serviceitem.edit', $service->id) }}"
                                            class="btn btn-primary shadow btn-xs sharp mr-1"><i
                                                class="fa fa-pencil"></i></a>

                                        <form action="{{ route('serviceitem.destroy', $service->id) }}" method="POST">
                                            @csrf @method('DELETE')
